...
```go
censysassetgraphsdkgo.WithServerURL("https://api.example.com")
```
...
```go
censysassetgraphsdkgo.WithTemplatedServerURL("https://{host}:{port}", map[string]string{
...
```go
censysassetgraphsdkgo.WithServerIndex(1)
```
...
```go
censysassetgraphsdkgo.WithRetryConfig(retry.Config{
    Strategy: "backoff",
...
```go
censysassetgraphsdkgo.WithTimeout(30 * time.Second)
```
